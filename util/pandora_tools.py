...
def make_json():
    ...
    with open(os.path.join(current_app.config['data_dir'], 'tokens.json'), 'r') as f:
    ...
    if os.path.exists(os.path.join(current_app.config['data_dir'], 'tokens.json')):
        import time
        os.rename(os.path.join(current_app.config['data_dir'], 'tokens.json'),
                  os.path.join(current_app.config['data_dir'],
                               'tokens.json.' + time.strftime("%Y%m%d%H%M%S", time.localtime())))

    # 将数据写入tokens.json
    with open(os.path.join(current_app.config['data_dir'], 'tokens.json'), 'w') as f:
        # 美化json
